...
The container maps its built-in Node.js and ADB binaries into the app's managed dependency directory, so no runtime download is required. It retries configured ADB endpoints every 15 seconds, so it will connect after the phone approves the key without a restart. The `adb-keys` volume preserves that authorization across image updates. Expose only port 8000 through the NAS HTTPS reverse proxy; do not publish Android's ADB port 5555 to the internet.
